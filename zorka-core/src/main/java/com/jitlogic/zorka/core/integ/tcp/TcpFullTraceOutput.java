 ...
 */
public class TcpFullTraceOutput extends ZorkaAsyncThread<SymbolicRecord> implements
        TraceStreamOutput {

    private static ZorkaLog log = ZorkaLogger.getLog(TcpFullTraceOutput.class);
     ...
     */
    private TcpFullTraceClientConnector conn;
     ...
     */
    public TcpFullTraceOutput(TraceWriter writer, String addr, int port, String hostname,
            String auth,
            int qlen, long packetSize, int retries, long retryTime, long retryTimeExp, int timeout,
            int interval) throws IOException {
        super("tcp-full-output", qlen, 1, interval);
        ...
        conn = new TcpFullTraceClientConnector(addr, port, timeout);

        this.writer = writer;
        this.os = new ByteArrayOutputStream(512 * 1024);
        this.writer.setOutput(this);

        log.info(ZorkaLogger.ZAG_CONFIG, "Configured tracer output: host=" + hostname + ", retries="
                + retries
                + ", retryTime=" + retryTime + ", packetSize=" + packetSize + ", addr=" + addr
                + ", port=" + port
                + ", timeout=" + timeout);
    }
...
//                }
    ...
    public void open() {
        ...
        try {
            ...
            log.error(ZorkaLogger.ZCL_STORE, "Error connecting " + conn.getAddr() + ":" + conn.
                    getPort()
    ...
    public synchronized void close() {
        ...
        try {
            ...
            log.error(ZorkaLogger.ZCL_STORE, "Error disconnecting " + conn.getAddr() + ":" + conn.
                    getPort(), e);
        }
    }
}
