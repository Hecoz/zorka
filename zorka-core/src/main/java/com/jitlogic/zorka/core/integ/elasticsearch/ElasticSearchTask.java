...
public class ElasticSearchTask implements Runnable, ZorkaRequestHandler {

    /* Logger */
    private final ZorkaLog log = ZorkaLogger.getLog(ElasticSearchTask.class);

    private final String index;
    private final ActiveCheckQueryItem item;
    ...
    public ElasticSearchTask(String index, ActiveCheckQueryItem item, ZorkaBshAgent agent, QueryTranslator translator, ConcurrentLinkedQueue<ElasticSearchCheckResult> responseQueue) {
        this.index = index;
        this.item = item;
        this.agent = agent;
    ...
    public void handleResult(Object rslt) {
        ...
        if (!value.equals(ZabbixActiveRequest.ZBX_NOTSUPPORTED)) { // TODO  && !value.equals("{\"data\":[]}")
            ElasticSearchCheckResult response = new ElasticSearchCheckResult();
            response.setKey(key);
